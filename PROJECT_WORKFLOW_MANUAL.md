# Codex Harness Template Project Workflow Manual

> 이 매뉴얼의 상대 경로는 `full_harness` 폴더 자체를 작업 루트로 열었을 때를 기준으로 작성되어 있습니다.  
> 독립 템플릿으로 복제해서 바로 쓰는 절차는 [README.md](README.md)를 먼저 보면 됩니다.
...
- 이 저장소에서 `Codex main agent`, `Codex sub-agent`, 여러 역할이 어떻게 함께 일하는지 설명합니다.
...
- 장점: 여러 Codex 세션과 여러 역할이 같은 프로젝트를 이어받아도 흐름이 유지됩니다.
...
## 2. Codex 세션, AI 모델, 에이전트란 무엇인가
이 부분은 초보자가 가장 자주 헷갈리는 부분입니다.  
`Codex 세션`, `AI 모델`, `에이전트`는 같은 말이 아닙니다.

### 2.1 Codex 세션이란 무엇인가
Codex 세션은 **하나의 작업 대화와 그 대화가 들고 있는 현재 맥락**입니다.

이 저장소에서 흔히 보게 되는 세션:
- 사용자와 직접 대화하는 `main` 세션
- 탐색/구현/검증을 맡는 `sub-agent` 세션

쉽게 말하면:
- 세션은 “지금 어느 맥락으로 일하고 있느냐”입니다.
- 같은 프로젝트라도 여러 Codex 세션이 병렬로 움직일 수 있습니다.
...
- “main 세션이 Planner 역할도 하고 Developer 역할도 했으니 같은 흐름이겠지”라고 착각
- “다른 Codex sub-agent가 했던 작업이니 main 세션이 자동으로 그대로 이해하겠지”라고 착각
...
## 3. Codex main agent와 Codex sub-agent의 차이
이 매뉴얼은 **Codex 하나를 기준으로 이 프로젝트를 어떻게 운영하느냐**만 설명합니다.
...
즉, **세션이 달라도 같은 문서 체계를 따르면 작업 품질을 맞출 수 있다**는 것이 이 템플릿의 핵심입니다.

### 운영 기준에서의 차이
이 저장소 기준으로 가장 중요한 차이는 **권한과 책임 범위**입니다.

**main agent**
- 보통 사용자와 직접 대화합니다.
- [`AGENTS.md`](AGENTS.md)부터 읽고 진입 순서를 잡습니다.
- 현재 라우팅, 다음 역할, 최종 상태 요약을 정리할 책임이 큽니다.

**sub-agent**
- main agent가 넘긴 좁은 범위의 탐색, 구현, 검증을 맡습니다.
- 자신에게 배정된 `Task ID`, `Scope`, `Session`, 필요 시 `Branch/Worktree`를 문서에 남깁니다.
- 결과를 main agent나 다음 역할이 이어받을 수 있게 handoff에 압축해서 남깁니다.

### 초보자가 이해해야 할 핵심
중요한 것은 “main agent냐 sub-agent냐”보다 아래입니다.
- 지금 어떤 역할로 들어왔는가
- 현재 상태 문서가 무엇을 말하는가
- lock과 handoff가 어떤 `Session` 이름으로 기록되어 있는가
...
### 사람이 다 기억하지 않아도 되는 이유: 자동 검사기
이 템플릿은 문서를 잘 쓰라고만 말하지 않고, **자동 검사기**로 기본 규칙을 다시 확인합니다.

대표 검사기:
- `python scripts/check_artifact_schema.py`
- `python scripts/check_planner_gate.py`
- `python scripts/check_current_state_sync.py`
- `python scripts/check_handoff_limits.py`
- `python scripts/check_closeout_readiness.py --mode day_wrap_up`
- `python scripts/check_closeout_readiness.py --mode version_closeout`
- `python scripts/run_harness_checks.py --phase planner|handoff|closeout`

초보자는 이렇게 이해하면 됩니다.
- 규칙의 원본은 여전히 [`workspace.md`](.agents/rules/workspace.md)입니다.
- 검사기는 새 규칙을 만드는 것이 아니라, **이미 있는 규칙을 기계적으로 확인하는 안전장치**입니다.
- 지금 이 저장소는 템플릿이므로 핵심 아티팩트 frontmatter에 `template_mode: true`가 들어 있습니다.
- 실제 프로젝트를 시작하면 Planner가 `template_mode: false`로 바꾸고, 그때부터 strict 검사가 본격적으로 동작합니다.
- 실제 전환 순서가 필요하면 [`PROJECT_START_CHECKLIST.md`](PROJECT_START_CHECKLIST.md)를 바로 따라가면 됩니다.
...
## 9. 병렬 작업에서 꼭 지켜야 할 것
여러 Codex 세션, 여러 AI 역할이 동시에 움직일 때는 아래를 꼭 지켜야 합니다.
...
- `Codex 세션`: Codex 안의 한 대화/작업 컨텍스트
