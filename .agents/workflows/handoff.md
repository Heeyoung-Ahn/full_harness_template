...
10. handoff 완료 직전 `python scripts/check_artifact_schema.py`, `python scripts/check_current_state_sync.py`, `python scripts/check_handoff_limits.py`를 실행한다.
11. 검사 중 하나라도 실패하면 handoff를 기록하지 말고 문서 정합성을 먼저 복구한다.
12. 마지막 handoff가 모호하거나 서로 충돌한다면 추측하지 말고 사용자에게 다음 역할을 확인받는다.
