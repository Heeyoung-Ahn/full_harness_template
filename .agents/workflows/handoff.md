...
9. Codex sub-agent를 썼다면 `Session`, `Branch`, `Worktree` 값을 `## Active Locks`와 handoff 기록에 같이 남긴다.
10. 해당 역할 workflow를 실행하고, 종료 시 `workspace.md`의 Handoff Protocol에 따라 `CURRENT_STATE.md`, `TASK_LIST.md`, 필요 시 `HANDOFF_ARCHIVE.md`를 정리한다.
11. handoff 완료 직전 `python scripts/check_artifact_schema.py`, `python scripts/check_current_state_sync.py`, `python scripts/check_handoff_limits.py`를 실행한다.
12. 검사 중 하나라도 실패하면 handoff를 기록하지 말고 문서 정합성을 먼저 복구한다.
13. 마지막 handoff가 모호하거나 서로 충돌한다면 추측하지 말고 사용자에게 다음 역할을 확인받는다.
