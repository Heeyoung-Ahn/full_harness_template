...
자동 검사:
- 실제 프로젝트로 전환하는 순간 Planner가 핵심 아티팩트 frontmatter의 `template_mode`를 `false`로 바꾸고 현재 상태 값으로 채웁니다.
- handoff 직전 `python scripts/check_artifact_schema.py`
- handoff 직전 `python scripts/check_planner_gate.py`
- 둘 중 하나라도 실패하면 handoff하지 않고 문서를 다시 닫거나 사용자 확인을 받습니다.
