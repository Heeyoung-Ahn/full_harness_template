...
- `TASK_LIST.md > ## Handoff Log`가 최근 8개 이내인지 확인하고, 초과분은 `HANDOFF_ARCHIVE.md`로 옮깁니다.
...
- `day_wrap_up` 직전에는 `python scripts/check_closeout_readiness.py --mode day_wrap_up`를 실행합니다.
- `version_closeout` 직전에는 `python scripts/check_closeout_readiness.py --mode version_closeout`를 실행합니다.
- closeout 검사 실패 시 archive나 초기화보다 먼저 lock, blocker, handoff 누락을 정정합니다.
